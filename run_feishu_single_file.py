...
# -*- coding: utf-8 -*-
"""
执行 feishu_unified_generator.py 处理单个文件并按依赖关系顺序执行测试用例

功能：
1. 执行 feishu_unified_generator.py 生成单个文件的测试用例
2. 根据依赖关系确定测试用例执行顺序
3. 按顺序执行测试用例
4. 生成 Allure 报告

使用方法：
    python run_feishu_single_file.py --file multiuploads/split_openapi/openapi_API/createCalendar.yaml
    或
    python run_feishu_single_file.py --file multiuploads/split_openapi/openapi_API/createCalendar.yaml --app-id YOUR_APP_ID --app-secret YOUR_APP_SECRET
"""
...
from utils.other_tools.feishu_unified_generator import FeishuUnifiedGenerator, OpenAPIFileInfo
...
class FeishuSingleFileGeneratorAndTestRunner:
    """飞书单个文件测试用例生成和执行器"""
    
    def __init__(self, file_path: str, app_id: str = None, app_secret: str = None, use_ai: bool = False):
    ...
    def step1_run_generator(self) -> bool:
        """步骤1: 执行 feishu_unified_generator.py 处理单个文件"""
        print("\n" + "=" * 60)
        print("步骤 1/4: 执行测试用例生成器（单文件模式）")
        print("=" * 60)
        ...
        if not self.file_path.suffix.lower() in ['.yaml', '.yml']:
            print(f"✗ 错误: 文件不是 YAML 格式: {self.file_path}")
            return False
        
        try:
            # 使用文件所在目录作为 folder_path，但只处理这个文件
            ...
            # 手动加载单个文件
            file_info = OpenAPIFileInfo(self.file_path, 0)
            if not file_info.load_yaml():
                print(f"✗ 错误: 无法加载 YAML 文件: {self.file_path}")
                return False
            
            # 设置生成器的文件列表为只包含这个文件
            self.generator.openapi_files = [file_info]
            
            # 获取 token
            print("\n获取 tenant_access_token...")
            self.generator.token = self.generator.get_tenant_access_token()
            if not self.generator.token:
                print("[WARN] 警告: 未获取到 token，部分用例可能需要手动填写 Authorization")
            
            # 按顺序生成用例（只处理这个文件）
            print(f"\n处理文件: {self.file_path.name}")
            print(f"API 路径: {file_info.api_path}")
            print("-" * 60)
            
            if not self.generator.generate_for_file(file_info, use_ai=self.use_ai):
                print(f"✗ 错误: 生成用例失败: {self.file_path.name}")
                return False
            
            # 生成 pytest 测试用例
            print("\n生成 pytest 测试用例...")
            self.generator._generate_test_cases_for_custom_path()
            
            # 创建测试用例初始化文件
            print("\n创建测试用例初始化文件...")
            self.generator._create_test_case_init_file()
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="执行 feishu_unified_generator.py 处理单个文件并按依赖关系顺序执行测试用例",
...
示例:
  ...
  # 指定 App ID 和 Secret
  python run_feishu_single_file.py \\
      --file multiuploads/split_openapi/openapi_API/createCalendar.yaml \\
      --app-id YOUR_APP_ID \\
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="使用 AI 大模型生成测试用例 YAML 文件（需要配置 AI API）"
    )
    
    args = parser.parse_args()
    
    try:
        runner = FeishuSingleFileGeneratorAndTestRunner(
            file_path=args.file,
            app_id=args.app_id,
